...
import me.ele.omniknight.common.tools.NetUtil;
import me.ele.omniknight.common.tools.ToastUtil;
import readcard.lauway.com.readcard.AppContext;

/**
 * 
 * 请求客户端
 * <p>
 * 其中的方法都是基于android-async-http-1.4.3.jar 中的异步网络请求实现的
...
public class RequestClient {
    ...
    /**
     * 定义一个异步网络客户端 默认超时为10秒 当超时，默认重连次数为5次 默认最大连接数为10个
     */
    ...
    public static boolean validateNetworkConnection(Context context) {
        if (!NetUtil.isConnectingToInternet(context)) {
            ToastUtil.showToastLong(context, "网络连接失败，请检查网络");
            return false;
        }
        return true;
    }
